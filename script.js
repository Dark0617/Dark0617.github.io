...
function initializeReceiptHandling() {
    ...
    if (!receiptInput || !imagePreview || !scanButton) {
...
function scanReceiptForAmount() {
    ...
    setTimeout(() => {
        ...
        if (amountField) {
            amountField.value = demoAmount;
            showNotification('Receipt scanned successfully!', 'success');
        } else {
            showNotification('Could not update amount field', 'error');
        }
    }, 2000);
}
...
function saveExpense(expense) {
    ...
    if (receiptImageData) {
        ...
        // Reset receipt data after saving
        receiptImage = null;
        receiptImageData = null;
        const imagePreview = document.getElementById('image-preview');
        if (imagePreview) {
...
function addNewExpenseWithPayment(name, amount, paymentId) {
    const currentUser = JSON.parse(sessionStorage.getItem('currentUser'));
    ...
    const expense = {
        ...
        name: name,
        amount: amount,
        category: document.getElementById('expense-category').value || 'Others',
...
// Add initializeFooter call to your initializeApp function
function initializeApp() {
    // ... existing code ...
